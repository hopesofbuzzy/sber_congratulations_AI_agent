...
4. **Data/Enrichment**
   - Добавлен локальный `demo-registry` организаций с `ИНН -> ОКВЭД/руководитель/сайт`.
   - Реализован реально работающий enrichment service без внешних API, пригодный для презентации и дальнейшей замены адаптером к боевым источникам.
   - Seed demo data теперь создаёт клиентов с `ИНН` и готовностью к enrichment.
5. **DevOps/QA**
   - Добавлены новые тесты на enrichment, feedback-loop, HTML-страницы и использование enriched-данных в генерации.
   - Поднят локальный `.venv` и прогнан полный backend test suite.
   - Актуализированы `README.md`, `AI_HANDOFF.md` и `docs/DECISIONS.md`.
...
3. `Run agent now`
4. Показать, что в `Clients` появились `ИНН/ОКВЭД/руководитель`
5. Показать персонализированные `Greetings`
6. Сохранить feedback менеджера
7. Показать `Deliveries` и outbox-артефакты
...
- **Контур демонстрации**: presentation-ready UI, управляемый сценарий demo flow, воспроизводимый seed данных.
